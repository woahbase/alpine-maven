...
VERSION   ?= $(call get_gh_version,apache/maven)
...
# from alpine-openjdk, ARM v7/v6 packages unavailable since 11
ifeq ($(JVVMAJOR), 11)
	SKIP_armv7l := 1
	SKIP_armhf  := 1
else ifeq ($(JVVMAJOR), 14)
	SKIP_armv7l := 1
	SKIP_armhf  := 1
else ifeq ($(JVVMAJOR), 17)
	SKIP_armv7l := 1
	SKIP_armhf  := 1
else ifeq ($(JVVMAJOR), 21)
	SKIP_armv7l := 1
	SKIP_armhf  := 1
endif
# -- }}}
